...
- [ ] Always-on smoke: create cartons from `/scan/pack/` workbench with scan/search, total quantity + quantity per carton, duplication to target total, cumulative batch actions, and final recap; verify unassigned `Disponible` cartons can still be assigned from `Vue Colis`.
