...
- couverture UI navigateur en progression (dashboard live, stock mutations+filtres, cartons/shipments-ready/shipments-tracking dedies avec table live + filtres suivi, archivage stale drafts + menu documents legacy sur shipments-ready, shipment create + creation colis inline, portal mutations), parite ecran stricte restante.
...
- actions metier critiques majeures des ecrans prioritaires davantage couvertes (stock mutations + filtres sur `scan/stock`, creation colis inline sur `shipment-create`, table live + filtres suivi sur `scan/shipments-tracking`, archivage stale drafts + menu documents legacy sur `scan/shipments-ready`),
