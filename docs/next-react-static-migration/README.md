...
- route `shipment-create` alignee sur les filtres de contacts par destination + sections progressives sans auto-selection + message d'absence expediteur + masquage de la zone creation colis/produit tant que selections incompletes + options select affichees en libelles metier (sans `id -`),
