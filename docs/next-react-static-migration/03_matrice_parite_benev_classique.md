...
| P1 | `/scan/shipment/` | `/app/scan/shipment-create/` | ecran present + mutations create(travail carton ou produit)/tracking/close branchees + liens admin destination/contacts + placeholders statiques retires + filtrage destination->contacts + message d'absence expediteur + blocage submit si selections incompletes | `GET/POST/PATCH /api/v1/ui/shipments*` | IN_PROGRESS |
...
- [x] workflow navigateur shipment-create avec destination hors scope expediteur -> message d'absence + submit bloque: `wms/tests/core/tests_ui.py::NextUiTests`
