...
| P1 | `/scan/shipment/` | `/app/scan/shipment-create/` | ecran present + mutations create(travail carton ou produit)/tracking/close branchees + liens admin destination/contacts + placeholders statiques retires + filtrage destination->contacts + message d'absence expediteur + sections progressives sans auto-selection (expediteur puis destinataire/correspondant puis zone creation colis/produit) + libelles options et labels metier sans prefixe technique `id -` ni suffixe `ID` + labels tracking/cloture metier (sans `Shipment ID`) | `GET/POST/PATCH /api/v1/ui/shipments*` | IN_PROGRESS |
| P1 | `/scan/shipments-tracking/` | `/app/scan/shipments-tracking/` | route dediee presente, table live + filtres (semaine/clos) + workflow tracking + etats visuels de cloture branches et testes + labels tracking/cloture metier (sans `Shipment ID`) | `GET /api/v1/ui/shipments/tracking/` + `POST /api/v1/ui/shipments/<id>/tracking-events/` | IN_PROGRESS |
...
- [x] workflow navigateur shipment-create avec labels/options metier sans prefixe technique `id -` ni suffixe `ID` (y compris tracking/cloture): `wms/tests/core/tests_ui.py::NextUiTests`
...
- [x] workflow navigateur route dediee suivi expedition (`/app/scan/shipments-tracking/`) + presence liste live + etats visuels cloture + labels metier tracking/cloture: `wms/tests/core/tests_ui.py::NextUiTests`
