...
| P1 | `/scan/shipment/` | `/app/scan/shipment-create/` | ecran present + mutations create(travail carton ou produit)/tracking/close branchees + liens admin destination/contacts + placeholders statiques retires + filtrage destination->contacts + message d'absence expediteur + sections progressives sans auto-selection (expediteur puis destinataire/correspondant puis zone creation colis/produit) | `GET/POST/PATCH /api/v1/ui/shipments*` | IN_PROGRESS |
