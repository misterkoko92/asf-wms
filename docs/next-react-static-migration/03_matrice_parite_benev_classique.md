...
|---|---|---|---|---|---|
| P1 | `/scan/dashboard/` | `/app/scan/dashboard/` | ecran present, KPI/timeline/actions + filtres destination/periode KPI + bloc KPI periode + bloc expeditions + bloc colis + bloc stock + bloc receptions/commandes + bloc suivi/alertes + bloc technique/queue email + blocages workflow + bloc suivi SLA + bloc graphique expeditions + bloc stock sous seuil branches en live API | `GET /api/v1/ui/dashboard/` | IN_PROGRESS |
...
| P1 | `/scan/shipment/` | `/app/scan/shipment-create/` | ecran present + mutations create(travail carton ou produit)/tracking/close branchees + liens admin destination/contacts + placeholders statiques retires + filtrage destination->contacts + message d'absence expediteur + sections progressives sans auto-selection (expediteur puis destinataire/correspondant puis zone creation colis/produit) + libelles options et labels metier sans prefixe technique `id -` ni suffixe `ID` + labels tracking/cloture metier (sans `Shipment ID`, terminologie `suivi`) + champ `Code produit (Creation)` francise + message de succes `Expedition #<id>` (sans `Shipment #`) | `GET/POST/PATCH /api/v1/ui/shipments*` | IN_PROGRESS |
| P1 | `/scan/shipments-tracking/` | `/app/scan/shipments-tracking/` | route dediee presente, table live + filtres (semaine/clos) + workflow tracking + etats visuels de cloture branches et testes + labels tracking/cloture metier (sans `Shipment ID`, terminologie `suivi`) | `GET /api/v1/ui/shipments/tracking/` + `POST /api/v1/ui/shipments/<id>/tracking-events/` | IN_PROGRESS |
...
- [x] workflow navigateur filtres stock (recherche + metadata produit + etat vide): `wms/tests/core/tests_ui.py::NextUiTests`
...
- [x] workflow navigateur shipment-create avec destination hors scope expediteur -> message d'absence + sections aval masqueees: `wms/tests/core/tests_ui.py::NextUiTests`
- [x] workflow navigateur shipment-create avec labels/options metier sans prefixe technique `id -` ni suffixe `ID` (y compris tracking/cloture en terminologie `suivi` + `Code produit`): `wms/tests/core/tests_ui.py::NextUiTests`
...
- [x] workflow navigateur route dediee suivi expedition (`/app/scan/shipments-tracking/`) + presence liste live + etats visuels cloture + labels metier tracking/cloture en terminologie `suivi`: `wms/tests/core/tests_ui.py::NextUiTests`
