...
|---|---|---|---|---|---|
| P1 | `/scan/dashboard/` | `/app/scan/dashboard/` | ecran present, KPI/timeline/actions + filtres destination/periode KPI + bloc KPI periode + bloc expeditions + bloc colis + bloc receptions/commandes + bloc graphique expeditions + bloc stock sous seuil branches en live API | `GET /api/v1/ui/dashboard/` | IN_PROGRESS |
...
- [x] workflow navigateur dashboard live (timeline + actions + filtres destination/periode KPI + bloc KPI periode + bloc expeditions + bloc colis + bloc receptions/commandes + bloc graphique expeditions + stock sous seuil): `wms/tests/core/tests_ui.py::NextUiTests`
