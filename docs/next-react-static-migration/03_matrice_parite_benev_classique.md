...
- [x] workflow navigateur dashboard live (timeline + actions + filtres destination/periode KPI + bloc KPI periode + bloc expeditions + bloc colis + bloc receptions/commandes + bloc suivi/alertes + bloc technique/queue email + bloc graphique expeditions + stock sous seuil): `wms/tests/core/tests_ui.py::NextUiTests`
