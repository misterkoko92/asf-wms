...
| P1 | `/scan/shipment/` | `/app/scan/shipment-create/` | ecran present + mutations create(travail carton ou produit)/tracking/close branchees + liens admin destination/contacts + placeholders statiques retires + filtrage destination->contacts + message d'absence expediteur + sections progressives sans auto-selection (expediteur puis destinataire/correspondant puis zone creation colis/produit) + libelles options et labels metier sans prefixe technique `id -` ni suffixe `ID` | `GET/POST/PATCH /api/v1/ui/shipments*` | IN_PROGRESS |
...
- [x] workflow navigateur shipment-create avec labels/options metier sans prefixe technique `id -` ni suffixe `ID`: `wms/tests/core/tests_ui.py::NextUiTests`
