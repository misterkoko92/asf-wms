...
- Recipient signup and portal recipient creation require structure compliance data plus a reception contact with title, first name, last name, at least one email, at least one phone, and required address fields. Public shipper and recipient signup both require structure city and country for nominal account requests.
- Recipient signup with `Autre escale` is a feasibility-only path: it reuses the selected profile and public email, shows only the stopover study payload, and must not duplicate account request structure fields.
...
- Shipper signup can store served stopovers selected as indicative review/history data in `PublicAccountRequest.shipper_stopover_indications`; this does not constrain future orders or grant route access. Selecting only `Autre escale` is feasibility-only like recipient signup; selecting served stopovers plus `Autre escale` keeps the nominal account request and embeds the feasibility payload for review.
