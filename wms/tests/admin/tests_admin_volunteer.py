...
from wms.admin import VolunteerAccountRequestAdmin, VolunteerProfileAdmin
...
class VolunteerAdminTests(TestCase):
    ...
    def test_volunteer_models_are_registered(self):
        ...
        self.assertIn(VolunteerAccountRequest, admin.site._registry)
...
class VolunteerAccountRequestAdminTests(TestCase):
    def setUp(self):
        self.factory = RequestFactory()
        self.site = AdminSite()
        self.superuser = get_user_model().objects.create_superuser(
    ...
    def test_approve_requests_marks_request_approved_and_creates_profile(self):
        ...
        admin_obj = VolunteerAccountRequestAdmin(VolunteerAccountRequest, self.site)

        with mock.patch.object(admin_obj, "message_user") as message_user_mock:
            admin_obj.approve_requests(
                self._request(),
                VolunteerAccountRequest.objects.filter(pk=account_request.pk),
            )
    ...
    def test_reject_requests_marks_request_rejected(self):
        account_request = VolunteerAccountRequest.objects.create(
            first_name="Lou",
            email="lou@example.com",
            status=VolunteerAccountRequestStatus.PENDING,
        ...
        with mock.patch.object(admin_obj, "message_user") as message_user_mock:
            admin_obj.reject_requests(
                self._request(),
                VolunteerAccountRequest.objects.filter(pk=account_request.pk),
            )

        account_request.refresh_from_db()
        self.assertEqual(account_request.status, VolunteerAccountRequestStatus.REJECTED)
        self.assertEqual(account_request.reviewed_by, self.superuser)
