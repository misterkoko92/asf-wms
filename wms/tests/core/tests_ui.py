...
class ScanUiTests(StaticLiveServerTestCase):
    ...
    def test_scan_pack_barcode_detector_restarts_without_video_load_reset(self):
        Product.objects.create(
            sku="UI-002",
            barcode="UI-BAR-002",
...
Object.defineProperty(navigator, 'mediaDevices', {
  value: {
    getUserMedia: async () => {
      ...
      if (window.__videoLoadCalls > 0) {
        throw new Error('camera-reset-blocked');
      }
...
"""
        with sync_playwright() as playwright:
            ...
            page.wait_for_function(
                "() => window.__getUserMediaCalls === 1 && window.__detectCalls === 1"
            )
            self.assertNotEqual(
                page.evaluate(
            ...
            self.assertEqual(page.evaluate("() => window.__videoLoadCalls"), 0)
            context.close()
            browser.close()

    def test_scan_pack_zxing_fallback_restarts_without_video_load_reset(self):
        Product.objects.create(
            sku="UI-003",
            barcode="UI-BAR-003",
        ...
        init_script = """
window.__scanCodes = ['UI-BAR-003', 'UI-BAR-003'];
window.__decodeFromConstraintsCalls = 0;
window.__videoLoadCalls = 0;
window.__zxingResetCalls = 0;
Object.defineProperty(navigator, 'mediaDevices', {
  value: {
...
"""
        with sync_playwright() as playwright:
            ...
            page.wait_for_function(
                "() => window.__decodeFromConstraintsCalls === 1"
                " && window.__scanCodes.length === 1"
                " && !document.getElementById('scan-overlay').classList.contains('active')"
            )

            scan_button.tap()
            page.wait_for_function(
                "() => window.__decodeFromConstraintsCalls === 2"
                " && window.__scanCodes.length === 0"
                " && !document.getElementById('scan-overlay').classList.contains('active')"
            )
            self.assertEqual(
