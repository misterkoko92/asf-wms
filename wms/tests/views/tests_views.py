...
class ScanViewTests(TestCase):
    ...
    def test_preparateur_whitelist_blocks_dashboard_and_shipments_views(self):
        ...
        order_select_response = self.client.get(reverse("scan:scan_preparateur_order_select"))
        last_carton_response = self.client.get(reverse("scan:scan_preparateur_last_carton"))
        ...
        self.assertEqual(order_select_response.status_code, 200)
        self.assertEqual(last_carton_response.status_code, 302)
    ...
    def test_scan_pack_creates_carton(self):
        ...
        self.assertContains(
            follow_response,
            'data-local-document-helper-minimum-version="0.1.2"',
        )
        self.assertContains(
            follow_response,
            (
    ...
    def test_scan_cartons_picking_renders_rows(self):
        carton_a = Carton.objects.create(code="C-PICK-A", status=CartonStatus.PICKING)
        carton_b = Carton.objects.create(code="C-PICK-B", status=CartonStatus.PICKING)
        CartonItem.objects.create(
            carton=carton_a,
            product_lot=ProductLot.objects.first(),
            quantity=2,
        ...
        self.assertContains(response, "C-PICK-A")
