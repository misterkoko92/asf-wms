...
class MigrationReviewItemStatus(models.TextChoices):
    OPEN = "open", "Ouvert"
...
class MigrationReviewItem(models.Model):
    legacy_contact = models.ForeignKey(
        "contacts.Contact",
        on_delete=models.PROTECT,
        related_name="migration_review_items_legacy",
        null=True,
        blank=True,
    )
    organization = models.ForeignKey(
        "contacts.Contact",
        on_delete=models.PROTECT,
        related_name="migration_review_items_organization",
        null=True,
        blank=True,
    )
    role = models.CharField(max_length=40, choices=OrganizationRole.choices)
    ...
    status = models.CharField(
        max_length=20,
        choices=MigrationReviewItemStatus.choices,
        default=MigrationReviewItemStatus.OPEN,
    )
    resolved_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="migration_review_items_resolved",
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["legacy_contact", "role", "reason_code"],
                condition=models.Q(status=MigrationReviewItemStatus.OPEN),
                name="wms_migration_review_item_single_open_issue",
            ),
        ]

    def __str__(self) -> str:
        label = self.organization or self.legacy_contact or f"item #{self.pk}"
        return f"Migration review {label} - {self.role} - {self.reason_code}"
