(() => {
  ...
  function setupPackLines() {
    ...
    const workbenchRoot = document.getElementById('pack-workbench');
    const workbenchForm = document.getElementById('pack-workbench-form');
    const quickGenerateButton = document.getElementById('pack-generate-cartons');
    const planHiddenFields = document.getElementById('pack-plan-hidden-fields');
    const planCountInput = document.getElementById('pack-plan-count');
    const planBody = document.getElementById('pack-draft-plan-body');
    const planSummaryCartons = document.querySelector('[data-pack-summary-cartons]');
    const planSummaryUnits = document.querySelector('[data-pack-summary-units]');
    const selectAllDraftCartons = document.getElementById('pack-select-all');
    const selectedCountEl = document.querySelector('[data-pack-selected-count]');
    const defaultOutputSelect = document.getElementById('pack-default-output-mode');
    const defaultDestinationSelect = document.getElementById('id_preassigned_destination');
    const defaultShipmentSelect = document.getElementById('id_shipment_reference');
    const defaultLocationSelect = document.getElementById('id_current_location');
    const batchFieldControls = workbenchRoot
      ? Array.from(workbenchRoot.querySelectorAll('[data-pack-batch-field]'))
      : [];
    const batchDuplicateButton = document.getElementById('pack-batch-duplicate');
    const batchDeleteButton = document.getElementById('pack-batch-delete');
    const rowDuplicateButton = document.getElementById('pack-row-duplicate');
    const rowDeleteButton = document.getElementById('pack-row-delete');
    const duplicateTargetInput = document.getElementById('pack-duplicate-target-count');
    const previewTitle = document.getElementById('pack-preview-title');
    const previewContent = document.getElementById('pack-preview-content');
    const previewAssignment = document.getElementById('pack-preview-assignment');
    const previewOutput = document.getElementById('pack-preview-output');
    const openFinalConfirmationButton = document.getElementById(
      'pack-open-final-confirmation'
    );
    const finalConfirmationModal = document.getElementById('pack-final-confirmation');
    const finalSummary = document.getElementById('pack-final-summary');
    ...
    const packDraftCartons = [];
    let packDraftCartonSequence = 0;
    let activePackDraftCartonId = null;
    const selectedPackDraftCartonIds = new Set();

    function formatPackOptionLabel(select, value, fallback = '-') {
      if (!value) {
        return fallback;
      }
      const option = select
        ? Array.from(select.options || []).find(candidate => candidate.value === String(value))
        : null;
      return option ? option.textContent.trim() || fallback : fallback;
    }

    function formatPackProductLabel(productCode) {
    ...
    function getPackOutputLabel(outputMode) {
      if (outputMode === 'available') {
        return 'Disponible';
      }
      if (outputMode === 'without_conditioning') {
        return 'Sans conditionner';
      }
      return '-';
    }

    function normalizePackDraftLine(line) {
      const quantity = parsePositiveInteger(line.quantity);
      if (!line.product_code || !quantity) {
        return null;
      }
      return {
        ...
        packFamilyOverride: line.pack_family_override || ''
      };
    }

    function getDefaultPackAssignment() {
      const shipmentReference =
        defaultShipmentSelect && defaultShipmentSelect.value
          ? defaultShipmentSelect.value
          : '';
      return {
        shipmentReference,
        preassignedDestination:
          shipmentReference || !defaultDestinationSelect ? '' : defaultDestinationSelect.value || '',
        currentLocation: defaultLocationSelect ? defaultLocationSelect.value || '' : ''
      };
    }

    function createPackDraftCartonState(lines, overrides = {}) {
      const assignment = getDefaultPackAssignment();
      const normalizedLines = lines.map(normalizePackDraftLine).filter(Boolean);
      return {
        id: `draft-${(packDraftCartonSequence += 1)}`,
        lines: normalizedLines,
        outputMode:
          overrides.outputMode ||
          (defaultOutputSelect && defaultOutputSelect.value) ||
          'available',
        preassignedDestination:
          overrides.preassignedDestination ?? assignment.preassignedDestination,
        shipmentReference: overrides.shipmentReference ?? assignment.shipmentReference,
        currentLocation: overrides.currentLocation ?? assignment.currentLocation,
        cartonFormatId:
          overrides.cartonFormatId ||
          (formatSelect && formatSelect.value ? formatSelect.value : 'custom')
      };
    }

    function getPackDraftCartonTotal(carton) {
      return carton.lines.reduce((total, line) => total + (line.quantity || 0), 0);
    }

    function formatPackDraftContent(carton) {
      if (!carton || !carton.lines.length) {
        return '-';
      }
      return carton.lines
        .map(line => `${formatPackProductLabel(line.productCode)} x${line.quantity}`)
        .join(', ');
    }

    function formatPackDraftAssignment(carton) {
      if (!carton) {
        return '-';
      }
      if (carton.shipmentReference) {
        return `Exp. ${carton.shipmentReference}`;
      }
      if (carton.preassignedDestination) {
        return formatPackOptionLabel(
          defaultDestinationSelect,
          carton.preassignedDestination,
          'Destination'
        );
      }
      return 'Libre';
    ...
    function clonePackDraftCarton(carton) {
      return createPackDraftCartonState(
        carton.lines.map(line => ({
          product_code: line.productCode,
          quantity: String(line.quantity),
          expires_on: line.expiresOn,
          pack_family_override: line.packFamilyOverride
        })),
        {
          outputMode: carton.outputMode,
          preassignedDestination: carton.preassignedDestination,
          shipmentReference: carton.shipmentReference,
          currentLocation: carton.currentLocation,
          cartonFormatId: carton.cartonFormatId
        }
      );
    }

    function getActivePackDraftCarton() {
      return packDraftCartons.find(carton => carton.id === activePackDraftCartonId) || null;
    }

    function getSelectedPackDraftCartons() {
      const selected = packDraftCartons.filter(carton =>
        selectedPackDraftCartonIds.has(carton.id)
      );
      if (selected.length) {
        return selected;
      }
      const active = getActivePackDraftCarton();
      return active ? [active] : [];
    }

    function updatePackWorkbenchSummary() {
      if (planSummaryCartons) {
        planSummaryCartons.textContent = String(packDraftCartons.length);
      }
      if (planSummaryUnits) {
        const totalUnits = packDraftCartons.reduce(
          (total, carton) => total + getPackDraftCartonTotal(carton),
          0
        );
        planSummaryUnits.textContent = String(totalUnits);
      }
      if (selectedCountEl) {
        selectedCountEl.textContent = String(selectedPackDraftCartonIds.size);
      }
      if (planCountInput) {
        planCountInput.value = String(packDraftCartons.length);
      }
      if (selectAllDraftCartons) {
        selectAllDraftCartons.checked =
          packDraftCartons.length > 0 &&
          selectedPackDraftCartonIds.size === packDraftCartons.length;
        selectAllDraftCartons.indeterminate =
          selectedPackDraftCartonIds.size > 0 &&
          selectedPackDraftCartonIds.size < packDraftCartons.length;
      }
    }

    function renderPackPreview() {
      const carton = getActivePackDraftCarton();
      if (!carton) {
        if (previewTitle) {
      ...
      const index = packDraftCartons.findIndex(candidate => candidate.id === carton.id);
      if (previewTitle) {
        previewTitle.textContent = `Colis ${index + 1}`;
      }
      if (previewContent) {
        previewContent.textContent = formatPackDraftContent(carton);
      }
      if (previewAssignment) {
        previewAssignment.textContent = formatPackDraftAssignment(carton);
      }
      if (previewOutput) {
        previewOutput.textContent = getPackOutputLabel(carton.outputMode);
      }
    }

    function appendPackTextCell(row, value) {
    ...
    function renderPackDraftPlan() {
      if (!planBody) {
        return;
      }
      planBody.replaceChildren();
      if (!packDraftCartons.length) {
        const emptyRow = document.createElement('tr');
        emptyRow.className = 'scan-pack-empty-row';
        const emptyCell = document.createElement('td');
        emptyCell.colSpan = 7;
        emptyCell.textContent = 'Aucun colis dans le plan.';
        emptyRow.appendChild(emptyCell);
        planBody.appendChild(emptyRow);
        activePackDraftCartonId = null;
        updatePackWorkbenchSummary();
        renderPackPreview();
        return;
      }

      packDraftCartons.forEach((carton, index) => {
        const row = document.createElement('tr');
        row.className = 'scan-pack-plan-row';
        row.classList.toggle('is-active', carton.id === activePackDraftCartonId);
        row.dataset.packDraftCartonId = carton.id;

        const selectCell = document.createElement('td');
        const checkbox = document.createElement('input');
        checkbox.type = 'checkbox';
        checkbox.className = 'pack-draft-select';
        checkbox.checked = selectedPackDraftCartonIds.has(carton.id);
        checkbox.setAttribute('aria-label', `Sélectionner colis ${index + 1}`);
        checkbox.addEventListener('change', () => {
          if (checkbox.checked) {
            selectedPackDraftCartonIds.add(carton.id);
            activePackDraftCartonId = carton.id;
          } else {
            selectedPackDraftCartonIds.delete(carton.id);
          }
          renderPackDraftPlan();
        });
        ...
        appendPackTextCell(row, `Colis ${index + 1}`);
        appendPackTextCell(row, formatPackDraftContent(carton));
        appendPackTextCell(row, String(getPackDraftCartonTotal(carton)));
        appendPackTextCell(row, formatPackDraftAssignment(carton));
        appendPackTextCell(row, getPackOutputLabel(carton.outputMode));

        const actionsCell = document.createElement('td');
        const focusButton = document.createElement('button');
        focusButton.type = 'button';
        focusButton.className = 'btn btn-tertiary btn-sm';
        focusButton.textContent = 'Aperçu';
    ...
    function generatePackDraftCartonsFromQuickGenerator() {
      if (!workbenchRoot) {
        return;
      }
      const rows = collectValues()
        .map(row => ({
          ...row,
          quantity: parsePositiveInteger(row.quantity),
          perCartonQuantity: parsePositiveInteger(row.per_carton_quantity)
        }))
        .filter(row => row.product_code && row.quantity);
      if (!rows.length) {
        window.alert('Ajoutez au moins un produit avec une quantité.');
        return;
      }
      const cartonCount = rows.reduce((maximum, row) => {
        const perCartonQuantity = row.perCartonQuantity || row.quantity;
        return Math.max(maximum, Math.ceil(row.quantity / perCartonQuantity));
      }, 1);
      const createdCartons = [];
      for (let cartonIndex = 0; cartonIndex < cartonCount; cartonIndex += 1) {
        const lines = [];
        rows.forEach(row => {
          const perCartonQuantity = row.perCartonQuantity || row.quantity;
          const remaining = row.quantity - cartonIndex * perCartonQuantity;
          const quantity = Math.min(perCartonQuantity, Math.max(remaining, 0));
          if (quantity <= 0) {
            return;
          }
          lines.push({
        ...
        if (lines.length) {
          createdCartons.push(createPackDraftCartonState(lines));
        }
      }
      if (!createdCartons.length) {
        window.alert('Le plan de colis est vide.');
        return;
      }
      packDraftCartons.push(...createdCartons);
      activePackDraftCartonId = createdCartons[0].id;
      selectedPackDraftCartonIds.clear();
      createdCartons.forEach(carton => selectedPackDraftCartonIds.add(carton.id));
      renderPackDraftPlan();
    }

    function duplicatePackDraftCartonToTargetCount(sourceCartonId, targetCount) {
      const source = packDraftCartons.find(carton => carton.id === sourceCartonId);
      const normalizedTargetCount = parsePositiveInteger(targetCount);
      if (!source || !normalizedTargetCount || normalizedTargetCount < 2) {
        window.alert('Indiquez un total de colis à 2 ou plus.');
        return;
      }
      const copiesToCreate = normalizedTargetCount - 1;
      const copies = [];
      for (let index = 0; index < copiesToCreate; index += 1) {
        copies.push(clonePackDraftCarton(source));
      }
      const sourceIndex = packDraftCartons.findIndex(carton => carton.id === sourceCartonId);
      packDraftCartons.splice(sourceIndex + 1, 0, ...copies);
      selectedPackDraftCartonIds.clear();
      copies.forEach(carton => selectedPackDraftCartonIds.add(carton.id));
      activePackDraftCartonId = copies.length ? copies[0].id : sourceCartonId;
      renderPackDraftPlan();
    }

    function applyPackBatchUpdateToSelectedCartons(fieldName, value) {
      if (!fieldName || !value) {
        return;
      }
      const targets = getSelectedPackDraftCartons();
      if (!targets.length) {
        window.alert('Sélectionnez au moins un colis.');
        return;
      }
      targets.forEach(carton => {
        if (fieldName === 'shipment_reference') {
          carton.shipmentReference = value;
          carton.preassignedDestination = '';
        } else if (fieldName === 'preassigned_destination') {
          carton.preassignedDestination = value;
          carton.shipmentReference = '';
        } else if (fieldName === 'current_location') {
          carton.currentLocation = value;
        } else if (fieldName === 'output_mode') {
          carton.outputMode = value;
        } else if (fieldName === 'carton_format_id') {
          carton.cartonFormatId = value;
        }
      });
      renderPackDraftPlan();
    }

    function deleteSelectedPackDraftCartons() {
      const targets = getSelectedPackDraftCartons();
      if (!targets.length) {
        return;
      }
      const targetIds = new Set(targets.map(carton => carton.id));
      for (let index = packDraftCartons.length - 1; index >= 0; index -= 1) {
        if (targetIds.has(packDraftCartons[index].id)) {
          packDraftCartons.splice(index, 1);
        }
      }
      selectedPackDraftCartonIds.clear();
      activePackDraftCartonId = packDraftCartons[0] ? packDraftCartons[0].id : null;
      renderPackDraftPlan();
    }

    function appendPackHiddenField(name, value) {
      if (!planHiddenFields) {
        return;
      }
      ...
      planHiddenFields.appendChild(input);
    }

    function serializePackExactPlanHiddenFields() {
      if (!planHiddenFields) {
        return;
      }
      planHiddenFields.replaceChildren();
      if (planCountInput) {
        planCountInput.value = String(packDraftCartons.length);
      }
      packDraftCartons.forEach((carton, cartonOffset) => {
        const cartonIndex = cartonOffset + 1;
        let name = `carton_${cartonIndex}_output_mode`;
        appendPackHiddenField(name, carton.outputMode || 'available');
        name = `carton_${cartonIndex}_preassigned_destination`;
        appendPackHiddenField(name, carton.preassignedDestination || '');
        name = `carton_${cartonIndex}_shipment_reference`;
        appendPackHiddenField(name, carton.shipmentReference || '');
        name = `carton_${cartonIndex}_current_location`;
        appendPackHiddenField(name, carton.currentLocation || '');
        name = `carton_${cartonIndex}_carton_format_id`;
        appendPackHiddenField(name, carton.cartonFormatId || '');
        name = `carton_${cartonIndex}_line_count`;
        appendPackHiddenField(name, String(carton.lines.length));
        carton.lines.forEach((line, lineOffset) => {
          const lineIndex = lineOffset + 1;
          name = `carton_${cartonIndex}_line_${lineIndex}_product_code`;
          appendPackHiddenField(name, line.productCode || '');
          name = `carton_${cartonIndex}_line_${lineIndex}_quantity`;
          appendPackHiddenField(name, String(line.quantity || ''));
          name = `carton_${cartonIndex}_line_${lineIndex}_expires_on`;
          appendPackHiddenField(name, line.expiresOn || '');
          name = `carton_${cartonIndex}_line_${lineIndex}_pack_family_override`;
          appendPackHiddenField(name, line.packFamilyOverride || '');
        });
      });
    }

    function renderPackFinalConfirmationSummary() {
      if (!finalSummary) {
        return;
      }
      finalSummary.replaceChildren();
      const totalUnits = packDraftCartons.reduce(
        (total, carton) => total + getPackDraftCartonTotal(carton),
        0
      );
      const headline = document.createElement('p');
      headline.className = 'scan-pack-final-summary-headline';
      headline.textContent = `${packDraftCartons.length} colis, ${totalUnits} unités à créer.`;
      finalSummary.appendChild(headline);

      const list = document.createElement('ul');
      list.className = 'scan-pack-final-summary-list';
      packDraftCartons.forEach((carton, index) => {
        const item = document.createElement('li');
        item.textContent = `Colis ${index + 1}: ${formatPackDraftContent(carton)} - ${formatPackDraftAssignment(carton)} - ${getPackOutputLabel(carton.outputMode)}`;
        list.appendChild(item);
      });
      finalSummary.appendChild(list);
      serializePackExactPlanHiddenFields();
    }
    ...
    const buildLine = (index, value, errors) => {
      ...
      quantityLabel.textContent = workbenchRoot ? 'Quantité totale' : 'Quantité';
      ...
      perCartonLabel.textContent = workbenchRoot ? 'Quantité par colis' : 'Qté / colis';
    ...
    if (workbenchRoot) {
      ...
      if (quickGenerateButton) {
        quickGenerateButton.addEventListener(
          'click',
          generatePackDraftCartonsFromQuickGenerator
        );
      }
      if (selectAllDraftCartons) {
        selectAllDraftCartons.addEventListener('change', () => {
          selectedPackDraftCartonIds.clear();
          if (selectAllDraftCartons.checked) {
            packDraftCartons.forEach(carton => selectedPackDraftCartonIds.add(carton.id));
          }
          renderPackDraftPlan();
        });
      }
      batchFieldControls.forEach(control => {
        control.addEventListener('change', event => {
          const fieldName = event.target.dataset.packBatchField || '';
          applyPackBatchUpdateToSelectedCartons(fieldName, event.target.value || '');
          event.target.value = '';
        });
      });
      if (rowDuplicateButton) {
        rowDuplicateButton.addEventListener('click', () => {
          const activeCarton = getActivePackDraftCarton();
          if (!activeCarton) {
            window.alert('Sélectionnez un colis à dupliquer.');
            return;
          }
          duplicatePackDraftCartonToTargetCount(
            activeCarton.id,
            duplicateTargetInput ? duplicateTargetInput.value : '2'
          );
        });
      }
      if (batchDuplicateButton) {
        batchDuplicateButton.addEventListener('click', () => {
          const targets = getSelectedPackDraftCartons();
          if (!targets.length) {
            window.alert('Sélectionnez au moins un colis à dupliquer.');
            return;
          }
          targets.forEach(carton => {
            duplicatePackDraftCartonToTargetCount(
      ...
      if (rowDeleteButton) {
        rowDeleteButton.addEventListener('click', deleteSelectedPackDraftCartons);
      }
      if (batchDeleteButton) {
        batchDeleteButton.addEventListener('click', deleteSelectedPackDraftCartons);
      }
      if (openFinalConfirmationButton) {
        openFinalConfirmationButton.addEventListener('click', () => {
          if (!packDraftCartons.length) {
            window.alert('Ajoutez au moins un colis au plan.');
            return;
          }
          renderPackFinalConfirmationSummary();
          if (finalConfirmationModal && window.bootstrap && window.bootstrap.Modal) {
            window.bootstrap.Modal.getOrCreateInstance(finalConfirmationModal).show();
          }
        });
      }
      if (workbenchForm) {
        workbenchForm.addEventListener('submit', event => {
          ...
          if (
            ...
            submitter.value === 'create_unknown_product'
          ) {
            return;
          }
          if (!packDraftCartons.length) {
