...
def _can_reuse_synced_contact_for_destination(contact, destination):
    return organization_can_be_reused_for_destination(contact, destination)
...
def sync_portal_recipient_graph(
    recipient,
    *,
    set_as_default=True,
    prefer_existing_structure=True,
...
def resolve_portal_recipient_party_contact(recipient):
    ...
    if synced_contact is None:
        ...
        synced_contact = result["synced_contact"] if result is not None else None
    if synced_contact is None:
        return None
