...
MESSAGE_RECIPIENT_BINDING_MISSING = (
    "Destinataire non autorise pour cet expediteur et cette escale."
)
...
def eligible_shippers_for_destination(destination):
    ...
    assignment_ids = ShipperScope.objects.filter(
        is_active=True,
    ).filter(
        _current_window_q(),
    ).filter(
        Q(all_destinations=True) | Q(destination=destination)
...
def eligible_recipients_for_shipper_destination(*, shipper_org, destination):
    ...
    recipient_ids = RecipientBinding.objects.filter(
        shipper_org=shipper_org,
        destination=destination,
        is_active=True,
    ).filter(_current_window_q()).values_list("recipient_org_id", flat=True)
...
def resolve_shipper_for_operation(*, shipper_org, destination):
    ...
    in_scope = ShipperScope.objects.filter(
...
def resolve_recipient_binding_for_operation(*, shipper_org, recipient_org, destination):
    ...
    recipient_assignment = OrganizationRoleAssignment.objects.filter(
    ...
    binding = RecipientBinding.objects.filter(
        shipper_org=shipper_org,
        recipient_org=recipient_org,
        destination=destination,
        is_active=True,
    ).filter(_current_window_q()).order_by("-valid_from", "-id").first()
    if binding is None:
